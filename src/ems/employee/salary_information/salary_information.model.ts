...
export class SalaryInformationModel {
  ...
  async createSalaryInformation(info: SalaryInformation): Promise<void> {
    const sql = `
    ...
    await this.db.execute(sql, [
      info.employee_id,
      info.payroll_frequency,
      info.base_salary,
    ]);
  }

  // Get salary information by employee ID
  async getSalaryInformationByEmployeeId(
    employeeId: number,
  ): Promise<SalaryInformation | null> {
    const sql = `
  ...
  async updateSalaryInformationById(
    id: number,
    info: SalaryInformation,
  ): Promise<void> {
    const sql = `
