...
export class IdentificationFinancialInformationController {
    ...
    async createIdentificationFinancialInformation(req: Request, res: ExpressResponse) {
        try {
            const { employee_id, pag_ibig_id, sss_id, philhealth_id, tin, bank_account_number } = req.body;

            if (employee_id === undefined || pag_ibig_id === undefined || sss_id === undefined ||
                philhealth_id === undefined || tin === undefined || bank_account_number === undefined) {
            ...
            const insertId = await this.model.createIdentificationFinancialInformation(info);
    ...
    async getIdentificationFinancialInformationByEmployeeId(req: Request, res: ExpressResponse) {
        try {
            const { employeeId } = req.params;
            const info = await this.model.getIdentificationFinancialInformationByEmployeeId(Number(employeeId));
    ...
    async updateIdentificationFinancialInformationById(req: Request, res: ExpressResponse) {
        try {
            const { id } = req.params;
            const { employee_id, pag_ibig_id, sss_id, philhealth_id, tin, bank_account_number } = req.body;
            ...
            await this.model.updateIdentificationFinancialInformationById(Number(id), info);
            return res.status(HttpStatus.OK.code).send(
                new Response(
                    HttpStatus.OK.code,
                    HttpStatus.OK.status,
    ...
    async deleteIdentificationFinancialInformationById(req: Request, res: ExpressResponse) {
        try {
            const { id } = req.params;
