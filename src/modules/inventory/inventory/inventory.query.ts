...
export class ItemModel {
    ...
    async create(item: Item): Promise<void> {
        ...
        VALUES (?, ?, ?, ?, ?)`;
        await this.db.execute(sql, [item.name, item.description, item.quantity, item.type, item.item_condition]);
    }
    ...
    async updateItemById(id: number, item: Partial<Item>): Promise<void> {
        ...
        await this.db.execute(sql, [item.name, item.description, item.quantity, item.type, item.item_condition, id]);
    }
